...
```{r, include=FALSE}
seeds = datashield.aggregate(conn, quote(seedBoundedToObject("D$age")))

seed_log_file = here::here("data/seeds.csv")
if (file.exists(seed_log_file)) {
  sds_old = read.csv(seed_log_file)
  sds = do.call(data.frame, c(list(
    time = as.character(Sys.time()),
    user = Sys.getenv()[["USER"]],
    os = Sys.info()[["release"]]),
    seeds))
  sds = rbind(sds, sds_old)
  write.csv(sds, file = seed_log_file, row.names = FALSE)
} else {
  sds = do.call(data.frame, c(list(
    time = as.character(Sys.time()),
    user = Sys.getenv()[["USER"]],
